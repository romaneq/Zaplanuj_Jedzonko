...
public class RecipeDao {
    ...
    private static final String USER_RECIPE_ADDED = "SELECT COUNT(*) as amount from recipe where admin_id = ?;";
    private static final String READ_RECIPE_BY_DESCRIPTION_QUERY = "SELECT * FROM recipe WHERE description = ?;";
    ...
    public int countUserRecipeAmount(int admin_id){
        try (Connection connection = DbUtil.getConnection();
             PreparedStatement statement = connection.prepareStatement(USER_RECIPE_ADDED)) {
    ...
    public Recipe readByDescription(String recipeDescription) {
        RecipeDao recipeDao = new RecipeDao();
        int recipeId = 0;
        try (Connection connection = DbUtil.getConnection();
             PreparedStatement statement = connection.prepareStatement(READ_RECIPE_BY_DESCRIPTION_QUERY)
        ) {
            statement.setString(1, recipeDescription);
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
